...
if( ! empty( $default_lang ) && ! empty( $added_langs ) ) { ?>
	<?php $sRewrite = $this->st_rewrite_type; ?>
	<div class="st-hr">
			...
			<table class="form-table permalink-structure">
				<tbody>
					...
					<tr>
						<th>
							<label>
								<input name="rewrite_type" id="rewrite_type_none" type="radio" value="none" <?php if ($sRewrite === 'none') { echo 'checked="checked"'; } ?>> <?php esc_attr_e('Content only', $this->plugin_name); ?>
							</label>
						</th>
						<td>
							<?php esc_attr_e('No additional URLs created for translated content.', $this->plugin_name); ?>
							<span class="gray-note"><?php esc_attr_e('Content provided back in your site tree as a pending with no language switcher options. Best for translations managed as grouped pages within the same site as original text.', $this->plugin_name); ?></span>
						</td>
					</tr>
				</tbody>
			</table>
