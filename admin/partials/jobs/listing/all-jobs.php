...
<div class="wrap">
	...
	if (isset($_GET['jk'])) {
		if ( !empty($_GET['jk']) && !empty($_GET['v']) == 'quote') {
			include_once $this->st_relative_path .'/admin/partials/straker-translations-admin-quote.php';
		} elseif ( ! empty($_GET['jk']) ) {
			include_once 'single-job.php';
		}
	} else {
	?>
	<?php include_once $this->st_relative_path .'/admin/partials/messages/sandbox-message.php'; ?>
	<h1><?php esc_attr_e( 'My Jobs', $this->plugin_name ); ?></h1>
	<?php
	if($this->straker_auth === false)  {

		include_once $this->st_relative_path .'/admin/partials/straker-translations-admin-register-button.php';
		...
		<?php
			...
			<div class=st-hr>
				...
				<?php	
					include_once $this->st_relative_path .'/admin/partials/messages/general-message.php';			
				...
				<a href="#" class="st-refresh-a" onclick="location.reload(true); return false;">
					<img width="32" height="32" src="<?php echo $this->plugin_absolute_path.'/admin/img/st-refresh-button-icon.png'; ?>" />
				</a>
